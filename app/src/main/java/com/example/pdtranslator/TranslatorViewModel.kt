...
data class TranslationEntry(
    ...
    var isModified: Boolean = false, // Represents a real, staged change
...
class TranslatorViewModel : ViewModel() {
            ...
            currentEntries.map { entry ->
                if (entry.key == key) {
                    ...
                    entry.copy(
                        ...
                        isIdentical = newTargetValue.isNotBlank() && newTargetValue == entry.sourceValue
                    )
                } else entry
            }
        }
    ...
    fun saveChangesToZip(resolver: ContentResolver, uri: Uri) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val modifiedFiles = _modifiedEntries.value

            if (modifiedFiles.isEmpty()) {
                return@launch // Nothing to save
            }

            resolver.openOutputStream(uri)?.use { fileOutputStream ->
                ZipOutputStream(fileOutputStream).use { zos ->
                    // Iterate ONLY over languages that have modifications
                    modifiedFiles.forEach { (langCode, modifications) ->
                        group.languages[langCode]?.let { langData ->
                            // Create final properties by merging original with modifications
                            val finalProps = Properties()
                            finalProps.putAll(langData.properties) // Load original
                            finalProps.putAll(modifications)      // Apply changes
            ...
            // Clear the modification cache for the saved languages and refresh UI
    ...
    private fun regenerateEntries() {
        ...
        viewModelScope.launch(Dispatchers.Default) {
            ...
            val newEntries = sortedKeys.map { key ->
                ...
                // This is the crucial part for tracking modifications
                ...
                val isIdentical = sourceValue.isNotBlank() && sourceValue == finalTargetValue
